 ...
 ******************************************************************************/
 ...
 package com.hi3project.dandelion.uib.modality.selector.jfl.fis.keyboard;
 ...
 *      24-jul-2014
 */
public class KeyboardModalityFIS extends AbstractModalitySelectionFIS
{

    @Override
    protected void setInputVariable(UserProfile user)
    {
        double hand = 0, arm = 0, finger = 0, language = 0, vision=0;
        if (user != null) {
            hand = user.getMotor().getHandPrecision();
            arm = user.getMotor().getArmPrecision();
            finger = user.getMotor().getFingerPrecision();
            language = user.getCognitive().getLanguageProduction();
            vision = user.getVision().getVisualAcuity();
        }
        ...
        getFIS().setVariable("user_language_production", language);
        getFIS().setVariable("user_visual_acuity", vision);
    }
    ...
    {
        Variable gestureGranularity = getFIS().getVariable("modality_keyboard");
        Modality gestureModality = new Modality(
                ModalityType.keyboard, gestureGranularity.getValue());
