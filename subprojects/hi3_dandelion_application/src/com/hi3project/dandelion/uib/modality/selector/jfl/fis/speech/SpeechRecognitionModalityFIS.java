 ...
 *      24-jul-2014
 */
public class SpeechRecognitionModalityFIS extends AbstractModalitySelectionFIS
{

    @Override
    protected void setInputVariable(UserProfile user)
    {
        double hearing = 0, langReception=0, langProduction=0, speechArt=0;
        if (user != null) {
            ...
            speechArt = user.getMotor().getSpeechArticulation();
        }
        ...
        getFIS().setVariable("user_speech_articulation", speechArt);
    }
    ...
    {
        Variable gestureGranularity = getFIS().getVariable("modality_speech_recognition");
        Modality gestureModality = new Modality(
                ModalityType.speech_recognition, gestureGranularity.getValue());
