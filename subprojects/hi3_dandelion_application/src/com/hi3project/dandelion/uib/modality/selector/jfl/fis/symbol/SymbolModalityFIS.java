 ...
 ******************************************************************************/
 ...
 package com.hi3project.dandelion.uib.modality.selector.jfl.fis.symbol;
 ...
 *      24-jul-2014
 */
public class SymbolModalityFIS extends AbstractModalitySelectionFIS
{

    @Override
    protected void setInputVariable(UserProfile user)
    {
        double visualAcuity=0, abstractSigns=0;
        if (user != null) {
            visualAcuity = user.getVision().getVisualAcuity();
            abstractSigns = user.getCognitive().getAbstractSigns();
        }
        
        getFIS().setVariable("user_visual_acuity", visualAcuity);
        getFIS().setVariable("user_cognitive_abstract_signs", abstractSigns);
    }
    ...
    {
        Variable gestureGranularity = getFIS().getVariable("modality_symbol");
        Modality gestureModality = new Modality(
                ModalityType.symbol, gestureGranularity.getValue());
